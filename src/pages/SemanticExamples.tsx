...
const ClassifierExample = () => {
    ...
    <div className="space-y-6">
      <div className="p-6 rounded-xl border border-border bg-card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold">Text Classification</h3>
            <p className="text-sm text-muted-foreground">Simple sentiment/topic classifier using coherence</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {['I am so happy today!', 'This is terrible news', 'Maybe we should think about it'].map(t => (
            <button
              key={t}
              onClick={() => setText(t)}
              className="px-3 py-1 rounded-full text-xs bg-muted hover:bg-primary/20 transition-colors"
        ...
        {classification && (
          <div className="space-y-4">
            <div className={`p-4 rounded-lg ${
              classification.category === 'positive' ? 'bg-primary/20 border-primary/30' :
              classification.category === 'negative' ? 'bg-destructive/20 border-destructive/30' :
              'bg-muted/50'
            } border`}>
              <p className="text-sm text-muted-foreground mb-1">Classification</p>
              <p className="text-2xl font-semibold capitalize">{classification.category}</p>
            ...
            <div className="space-y-2">
              {classification.scores.map(s => (
                <div key={s.category} className="flex items-center gap-4">
                  <span className="w-20 text-sm capitalize">{s.category}</span>
  ...
  positive: ['happy', 'joy', 'love', 'wonderful'],
  negative: ['sad', 'angry', 'hate', 'terrible'],
  neutral: ['think', 'maybe', 'could', 'perhaps'],
};

function classify(text, backend) {
  const textState = backend.primesToState(backend.encode(text));
  
  const scores = Object.entries(categories).map(([cat, words]) => {
    const catState = backend.primesToState(
      words.flatMap(w => backend.encode(w))
    );
    return { category: cat, score: textState.coherence(catState) };
  });
  ...
  return scores[0];
        ...
        language="javascript"
        title="semantic/07-classifier.js"
      />
    </div>
  );
};
